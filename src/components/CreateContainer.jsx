...
import { MdAttachMoney, MdCloudUpload, MdDelete, MdDescription } from 'react-icons/md'
import { categories } from '../utils/data';
import Loader from './Loader';
import { deleteObject, getDownloadURL, ref, uploadBytesResumable } from 'firebase/storage';
import { storage } from '../firebase.config';
import { getAllCars, saveItem } from '../utils/firebaseFunctions';
...
const CreateContainer = () => {
  ...
  const saveDetails = () => {
    setIsLoading(true);
    try {
      if (!title || !model || !imageAsset || !price || !description || !category) {
        setFields(true);
    ...
    <div className='w-full min-h-screen flex items-center justify-center'>
      ...
      items-center justify-center gap-4'>
        ...
        <div className='w-full py-2 border-b border-gray-300 flex items-center gap-2'>
          <AiFillCar className='text-xl text-gray-700' />
          <input
            type="text"
            required
            value={title}
        ...
        <div className='w-full'>
          <select onChange={(e) => setCategory(e.target.value)} className="outline-none w-full text-base border-b-2 border-gray-200 p-2 rounded-md cursor-pointer  " >
            ...
            {categories && categories.map((car) => (
              <option
                key={car.id}
                className="text-base border-0 outline-none capitalize bg-white text-headingColor"
                value={car.urlParamName}
              >
                {car.name}
              </option>
            ))}
